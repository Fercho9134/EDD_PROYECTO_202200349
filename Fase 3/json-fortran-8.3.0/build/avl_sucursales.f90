module avl_m
    ...
    type :: nodo_avl
        !nodo representa a una sucursal
    !Valor es el id de la sucursal
        integer :: valor
        integer :: altura = 1
        character(len=:), allocatable :: departamento
        character(len=:), allocatable :: direccion
        integer :: impresorasMantenimiento = 0
        character(len=:), allocatable :: contrasena
        type(TablaHash) :: tabla_tecnicos
...
contains
    ...
    recursive subroutine imprimirRec(raiz, nombre, io)
        ...
        if(associated(raiz)) then
            !"Nodo_uuid"[Label="1"]
            write(io, *) '"Nodo'//nombre//'"[label= "Sucursal', raiz%valor, '"]'
    ...
    function contarNodos(self) result(res)
        class(avl), intent(in) :: self
        type(nodo_avl), pointer :: nodo_iterador
        integer :: res
        ...
        if(associated(self%raiz)) then
            nodo_iterador => self%raiz
            res = contarNodosRec(nodo_iterador)
        end if
    end function contarNodos

    recursive function contarNodosRec(raiz) result(res)
        type(nodo_avl), pointer :: raiz
        integer :: res
        ...
        if(associated(raiz)) then
            res = 1 + contarNodosRec(raiz%izquierda) + contarNodosRec(raiz%derecha)
        end if
    end function contarNodosRec
